...
const UploadDoc = () => {
  ...
  const handleCreateChatbot = async (event: React.FormEvent<HTMLFormElement>) => {
  ...
  return (
    ...
    <Box>
      ...
      <button style={{ marginRight: '4px' }} disabled={uploadedFileName ? true : false} className='custom-button' onClick={handleSubmit}>Upload Docs</button>
      <button disabled={!file || file?.length === 0 ? true : false} className='custom-button' onClick={() => {setFile([]); setUploadedFileName(null)}}>Clear Docs</button>
    </Box>
    <Box component="form" onSubmit={handleCreateChatbot} noValidate sx={{ mt: 1 }}>
    <TextField
      margin="normal"
      required
      fullWidth
